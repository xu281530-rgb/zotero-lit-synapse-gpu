...
pref-hybrid-gpu-status-disabled = GPU 加速已关闭。
pref-hybrid-gpu-status-preparing = 正在准备 GPU 资源...
pref-hybrid-gpu-status-loading = 正在将向量加载到显存（{ $loaded }/{ $total }）...
pref-hybrid-gpu-status-available = GPU 已可用：{ $device }，显存中常驻 { $vectors } 条向量。
pref-hybrid-gpu-status-fallback = GPU 不可用，已回退 CPU（{ $code }）：{ $reason }
