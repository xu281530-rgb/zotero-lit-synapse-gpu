...
for (const key of retiredKeys) {
  values.set(`extensions.zotero.zotero-mcp-plugin.${key}`, "old-value");
}
for (const key of [
  "mcp.server.port",
  "write.enabled",
  ...
  values.set(`extensions.zotero.zotero-mcp-plugin.${key}`, `keep-${key}`);
}
...
assert.deepEqual(
  cleared,
  retiredKeys.map((key) => `extensions.zotero.zotero-mcp-plugin.${key}`),
);
for (const key of retiredKeys) {
  assert.equal(
    values.has(`extensions.zotero.zotero-mcp-plugin.${key}`),
    false,
  );
}
for (const key of [
  ...
  assert.equal(
    values.get(`extensions.zotero.zotero-mcp-plugin.${key}`),
    `keep-${key}`,
  );
}
