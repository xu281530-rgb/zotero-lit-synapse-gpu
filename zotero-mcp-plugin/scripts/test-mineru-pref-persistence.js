...
const prefPrefix = "extensions.zotero.zotero-mcp-plugin.mineru.";
...
function testMainPreferencesPreserveCustomURL() {
  ...
  const controls = {
    "#zotero-prefpane-zotero-mcp-plugin-mineru-mode": createControl(),
    "#zotero-prefpane-zotero-mcp-plugin-mineru-base-url": createControl(),
    "#zotero-prefpane-zotero-mcp-plugin-mineru-model-version":
      createControl("vlm"),
  };
  const doc = {
  ...
  const context = {
    ...
    require(moduleName) {
      if (moduleName.endsWith("package.json")) {
        return { config: { addonRef: "zotero-mcp-plugin" } };
      }
    ...
    Zotero: { Prefs: prefs },
    config: { addonRef: "zotero-mcp-plugin" },
    getString: (key) => key,
    ztoolkit: { log() {} },
  };
  ...
  assert.equal(
    controls["#zotero-prefpane-zotero-mcp-plugin-mineru-base-url"].value,
    customLocalURL,
  );
}
