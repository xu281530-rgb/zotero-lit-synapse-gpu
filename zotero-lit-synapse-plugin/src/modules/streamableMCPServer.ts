...
const PREF_WRITE_ENABLED = 'extensions.zotero.zotero-lit-synapse.write.enabled';
const PREF_WRITE_CONFIRM =
  'extensions.zotero.zotero-lit-synapse.write.confirmBeforeMutation';
const PREF_ALLOW_FILE_IMPORT =
  'extensions.zotero.zotero-lit-synapse.write.allowFileImport';
...
const WRITE_DISABLED_MESSAGE =
  'Write operations are currently disabled. Please go to Zotero → Tools → Add-ons → Zotero LitSynapse → Preferences, and enable "Write Operations" to use this feature.';
 ...
 */
/** Where the stage-by-stage funnel is served from. */
export const WORKFLOW_GUIDE_URI = 'zotero://guide/workflow';

/**
 * The retrieval funnel, stage by stage.
 *
 * This used to live inside the `initialize` response's `instructions`, which
 * the client puts in its system prompt - so all of it was re-sent on every
 * turn, including the stages of a workflow a given conversation never enters.
 * It is served from zotero://guide/workflow instead, and `instructions` keeps
 * the orientation that says which stage comes next.
 *
 * Moved verbatim. Every rule the stages state is either enforced by the
 * server, which teaches the fix in its own error, or repeated in the tool's
 * own `doctrine`; none of it was condensed on the way here.
 */
export const MCP_WORKFLOW_GUIDE = `STAGE 0 - decide where to look (get_collections, only when it helps):
...
15. Answer the user first. Then, for EACH paper whose passages you genuinely read and used, call wiki_update_reading_note with that paper's itemKey, readChunkIds listing the chunkId of every passage you actually used, the same domain and expertRole you searched it with, and ONE readingRecord containing only what this turn established. Three papers read means three calls. The server cites the turn's chunks, audits the new record against them immediately, numbers it, and appends it without modifying any older record. Cite the chunk number beside every fact so the trail back to the source survives; if later text corrects an older entry, append a correction that names its record number. A record DISTILS what it read rather than compressing it: it says how many chunks the turn covered, cites every one of them, keeps the core reasoning and the conclusions, and carries each value with its unit and its measured condition. Naming a chunk and then saying only what it was about records a table of contents, not a reading.
...
19. Once every chunk is covered, reset any provisional expert from the paper's metadata and abstract, then call wiki_update_reading_note with finalSynthesis true and ONE macroSummary. It is appended after all immutable records and checked sentence by sentence against the chunks it cites. Distil the paper as a whole - its core question, core methods, principal findings, mechanism and limits. Do not paste the records end to end, reproduce full parameter tables, or preserve numbers mechanically; include a value only when it is necessary to express a core finding or distinguish an important condition. The complete details remain in the immutable records directly above it. Next call wiki_record_concepts with final true, then wiki_prepare_update with the five-axis Wiki Review and one claimVerdict for every Claim recalled by this paper's Evidence, then wiki_commit. Use UPDATE_CLAIM for overstated wording and MARK_CONFLICT for factual contradictions. A question-driven reading may produce this macro summary at 100% coverage but its Evidence remains capped at section_read; paper_reviewed still requires the explicit full-text path through wiki_build_from_paper.
...
如果说明要求使用的工具在当前 MCP 工具列表中不存在，请提示用户到 Zotero LitSynapse 设置中启用对应功能，不要尝试调用不存在的工具。
读取顺序与各阶段的详细做法在资源 zotero://guide/workflow 里；每个工具自己的方法在 zotero://tool/<name> 里。
READ zotero://guide/workflow BEFORE your first retrieval call in a session. It is the stage-by-stage
procedure this server is built around - which tool comes next, what each stage may and may not do, and
where to stop. Read it once; it holds for the session.

The stages, in order: 0 get_collections (scope, only when it helps) -> 1 hybrid_search (candidates) ->
2 get_item_abstract (triage, only where needed) -> 3 search_fulltext (one paper per call) ->
4 wiki_update_reading_note, then wiki_prepare_update + wiki_commit (keep what was learned).`,
    });
  }
      ...
      resources: [
        {
          ...
          name: 'Retrieval workflow — the five stages',
          description:
            'Which tool comes next, what each stage may and may not do, and where to stop. Read once per session before the first retrieval call.',
          mimeType: 'text/markdown',
        },
        ...projectDoctrineResources(this.getAvailableTools()),
      ],
    });
  }
  ...
  private handleResourcesRead(request: MCPRequest): MCPResponse {
    ...
    if (uri === WORKFLOW_GUIDE_URI) {
      return this.createResponse(request.id ?? null, {
        contents: [
          { uri, mimeType: 'text/markdown', text: MCP_WORKFLOW_GUIDE },
        ],
      });
    }
      ...
      switch (name) {
        ...
        case 'create_collection': {
          const writeEnabledCC = Zotero.Prefs.get(
            'extensions.zotero.zotero-lit-synapse.write.enabled',
            true,
          );
  ...
  private async callSemanticStatus(): Promise<any> {
    try {
      ...
      if (int8Status?.needed) {
        message += `. NOTE: ${int8Status.count}/${int8Status.total} stored vectors predate Int8 quantisation, so searches over them run on the slower Float32 path. This is not something you can fix from here and it does not affect result quality — searches work normally. If the user asks about it, tell them to rebuild the semantic index from Zotero → Preferences → Zotero LitSynapse → Search; newly indexed vectors are always quantised.`;
      }
      ...
      switch (action) {
        ...
        case 'import': {
          ...
          if (!isFileImportAllowed()) {
            throw new Error(
              'File import from local paths is disabled. Enable "Allow File Import" in the Zotero LitSynapse preferences to use write_item action "import".',
            );
          }
