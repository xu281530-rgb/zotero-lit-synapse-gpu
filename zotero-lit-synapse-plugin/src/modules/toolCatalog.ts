...
export function buildToolCatalog(): ToolDefinition[] {
  return [
  {
    ...
    description: [
      ...
      '',
      'THIS CALL DOES NOT FINISH THE JOB. A candidate row is a title, a rank and a snippet - enough to decide WHICH papers are worth reading and never enough to answer a question about what they FOUND. When the user asked which literature is relevant, answer from these rows and stop. Otherwise continue: get_item_abstract on the few worth pursuing, then search_fulltext on ONE of them at a time for the passages that actually carry the answer, then record what you read. Answering a substantive question straight off these rows is the failure this funnel exists to prevent - it reads like an answer and is built out of titles. The full procedure for each step, including how to re-fit the query to one paper, is in this tool\'s METHOD resource.',
    ].join('\n'),
    doctrine: [
      ...
      '',
      'A CURSOR DOES NOT LIVE FOREVER, which matters most in exactly the case that pages the most. The ranking a cursor names is held for 15 MINUTES, and only the 5 MOST RECENT searches are kept; a sixth search evicts the oldest, and changing a Wiki preference between pages invalidates the cursor too. A literature review that fans out into six sub-questions and then comes back for page 2 of the first one will find that cursor gone, and reading several papers in between can spend the 15 minutes on its own. The refusal says so explicitly rather than returning wrong rows. When it happens, re-run the search and page it out THEN, before starting the next sub-question: finish paging one ranking while it is live rather than collecting first pages and returning later.',
    ...
    inputSchema: {
      ...
      properties: {
        ...
        wikiMinScore: {
          type: 'number',
          description: 'Wiki-route relevance floor 0-1. Gates the third route only, and only matters when the user has taken the Wiki RRF weight off its default 0 — otherwise that route contributes nothing whatever this is set to. Same "stricter only" rule as the other two floors. The server has always accepted this; it was simply never documented, so a caller could neither tighten it nor tell it existed.'
        },
        language: {
          type: 'string',
          enum: ['zh', 'en', 'all', 'auto'],
  ...
  // Ordered reading of one document's indexed body
  {
    ...
    inputSchema: {
      ...
      // Same rule as hybrid_search's, and now expressed the same way: itemKey
      // starts a document, cursor continues one, and exactly one of them has
      // to be there. It was written only in the itemKey description, so a
      // client validating against the schema saw a tool with no required
      // argument at all and a call with neither was shaped like a legal one.
      required: [],
      anyOf: [{ required: ['itemKey'] }, { required: ['cursor'] }],
    },
  },
  {
