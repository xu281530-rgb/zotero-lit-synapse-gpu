...
function checkAuthorization(headers: RequestHeaders): MCPHttpResponse | null {
  ...
  if (!expected || expected.length < 32) {
    ...
    return jsonError(
      ...
      "Service Unavailable",
      {
        ...
        detail:
          "Remote access is enabled but no valid access token exists. Open the Zotero LitSynapse preferences and regenerate the access token.",
      },
    );
  }
  ...
  if (!presented) {
    return jsonError(
      ...
      { "WWW-Authenticate": 'Bearer realm="zotero-lit-synapse"' },
    );
  }
  ...
  if (!constantTimeStringEqual(presented, expected)) {
    return jsonError(
      ...
      { "WWW-Authenticate": 'Bearer realm="zotero-lit-synapse", error="invalid_token"' },
    );
  }
