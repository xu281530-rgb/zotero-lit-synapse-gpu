...
export function createWikiTermsView(options: {
  ...
  list.id = "zotero-lit-synapse-wiki-terms-list";
  const detail = element(doc, "main", "zmp-wiki-terms-detail");
  detail.id = "zotero-lit-synapse-wiki-terms-detail";
  ...
  const renderList = () => {
    ...
    search.id = "zotero-lit-synapse-wiki-terms-search";
    ...
    search.addEventListener("input", () => {
      ...
      const next = list.querySelector<HTMLInputElement>(
        "#zotero-lit-synapse-wiki-terms-search",
      );
      if (next) {
        next.focus();
