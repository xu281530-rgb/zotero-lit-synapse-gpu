...
export class MinerUService {
  ...
  private getCacheRoot(): string {
    return PathUtils.join(Zotero.DataDirectory.dir, "zotero-lit-synapse", "mineru");
  }
  ...
  async testConnection(): Promise<{ ok: boolean; message: string }> {
    ...
    try {
      ...
      const response = await resolveFetch()(
        `${config.baseURL}/api/v4/extract-results/batch/zotero-lit-synapse-connectivity-check`,
        {
          method: "GET",
          headers: {
            Accept: "*/*",
