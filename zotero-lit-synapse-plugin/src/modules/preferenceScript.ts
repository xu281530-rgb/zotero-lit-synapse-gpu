...
export async function registerPrefsScripts(_window: Window) {
  ...
  try {
    ...
    if (doc) {
      ...
      // 特别检查服务器启用元素
      const serverEnabledElement = doc.querySelector('#zotero-prefpane-zotero-lit-synapse-mcp-server-enabled');
...
function bindPrefEvents() {
  ...
  if (serverEnabledCheckbox) {
    ...
    serverEnabledCheckbox.addEventListener("change", () => {
      ...
      Zotero.Prefs.set("extensions.zotero.zotero-lit-synapse.mcp.server.enabled", checked, true);
        ...
        if (httpServer) {
          if (checked) {
            if (!httpServer.isServerRunning()) {
              const portPref = Zotero.Prefs.get("extensions.zotero.zotero-lit-synapse.mcp.server.port", true);
  ...
  if (portInput) {
    const savedPort = Zotero.Prefs.get("extensions.zotero.zotero-lit-synapse.mcp.server.port", true);
  ...
  portInput?.addEventListener("change", () => {
    if (portInput) {
      ...
      if (isNaN(port) || port < 1024 || port > 65535) {
        ...
        const originalPort = Zotero.Prefs.get("extensions.zotero.zotero-lit-synapse.mcp.server.port", true) || 23120;
        portInput.value = originalPort.toString();
      } else {
        Zotero.Prefs.set("extensions.zotero.zotero-lit-synapse.mcp.server.port", port, true);
      }
    }
  });

  // Bind HTML toggle switches (these need manual pref sync since they're not XUL checkboxes)
  bindHtmlCheckbox(doc, `#zotero-prefpane-${config.addonRef}-mcp-server-allow-remote`, "extensions.zotero.zotero-lit-synapse.mcp.server.allowRemote");
  bindHtmlInput(doc, `#zotero-prefpane-${config.addonRef}-mcp-server-auth-token`, "extensions.zotero.zotero-lit-synapse.mcp.server.authToken");
  bindHtmlCheckbox(doc, `#zotero-prefpane-${config.addonRef}-write-confirm`, "extensions.zotero.zotero-lit-synapse.write.confirmBeforeMutation");
  bindHtmlCheckbox(doc, `#zotero-prefpane-${config.addonRef}-file-import-enabled`, "extensions.zotero.zotero-lit-synapse.write.allowFileImport");
  bindHtmlCheckbox(doc, `#zotero-prefpane-${config.addonRef}-expose-file-paths`, "extensions.zotero.zotero-lit-synapse.privacy.exposeFilePaths");
  ...
  regenerateTokenButton?.addEventListener('click', () => {
    ...
    Zotero.Prefs.set("extensions.zotero.zotero-lit-synapse.mcp.server.authToken", token, true);
  ...
  remoteToggle?.addEventListener('change', () => {
    try {
      if (remoteToggle.checked) {
        let token = String(Zotero.Prefs.get("extensions.zotero.zotero-lit-synapse.mcp.server.authToken", true) || '').trim();
        if (token.length < 32) {
  ...
  bindHtmlCheckbox(doc, `#zotero-prefpane-${config.addonRef}-semantic-auto-update`, "extensions.zotero.zotero-lit-synapse.semantic.autoUpdate");
  ...
  generateButton?.addEventListener("click", () => {
    try {
      ...
      const serverName = serverNameInput?.value?.trim() || "zotero-lit-synapse";
...
function bindHybridSearchSettings(doc: Document) {
  const P = "extensions.zotero.zotero-lit-synapse.hybrid.";
  const ref = config.addonRef;
  ...
  bindBoundedNumber(
    ...
    "extensions.zotero.zotero-lit-synapse.embedding.maxBatchItems",
    1,
    2048,
    20,
  );
...
function bindMinerUSettings(doc: Document) {
  const P = "extensions.zotero.zotero-lit-synapse.mineru.";
  const ref = config.addonRef;
...
function initIntegratedTranslationPreferences(win: Window) {
  const tryInit = () => {
    try {
      ...
      const rootNode = win?.document?.querySelector('#zotero-lit-synapse-preferences');
...
function bindTranslationSettings(doc: Document) {
  const L = "extensions.zotero.zotero-lit-synapse.llm.";
  const T = "extensions.zotero.zotero-lit-synapse.translation.";
...
function bindHelpPopovers(doc: Document) {
  ...
  const place = (mark: HTMLElement) => {
    ...
    const pane = (doc.querySelector(`#${config.addonRef}-preferences`) ??
  ...
  for (const mark of Array.from(
    doc.querySelectorAll('.zmp-q'),
  ) as HTMLElement[]) {
    mark.addEventListener('mouseenter', () => place(mark));
    mark.addEventListener('focusin', () => place(mark));
  }
}
...
const PREF_SERVER_ENABLED = 'extensions.zotero.zotero-lit-synapse.mcp.server.enabled';
...
function bindEmbeddingSettings(doc: Document) {
  ...
  if (providerSelect) {
    const savedApiBase = Zotero.Prefs.get("extensions.zotero.zotero-lit-synapse.embedding.apiBase", true) as string;
  ...
  initValue(apiBaseInput, "extensions.zotero.zotero-lit-synapse.embedding.apiBase", "https://api.openai.com/v1");
  initValue(apiKeyInput, "extensions.zotero.zotero-lit-synapse.embedding.apiKey", "");
  ...
  initValue(modelInput, "extensions.zotero.zotero-lit-synapse.embedding.model", "");
  initValue(dimensionsInput, "extensions.zotero.zotero-lit-synapse.embedding.dimensions", "512");
  ...
  const updateDimensionsVisibility = () => {
    ...
    if (dimensionsRow && testResult) {
      if (!supportsCustom) {
        ...
        const detectedDims = Zotero.Prefs.get("extensions.zotero.zotero-lit-synapse.embedding.detectedDimensions", true);
        if (detectedDims) {
    ...
    providerSelect.addEventListener("change", () => {
      ...
      if (provider !== "custom" && EMBEDDING_PROVIDER_PRESETS[provider]) {
        ...
        if (apiBaseInput) {
          ...
          Zotero.Prefs.set("extensions.zotero.zotero-lit-synapse.embedding.apiBase", preset.apiBase, true);
        }
  ...
  bindSave(apiBaseInput, "extensions.zotero.zotero-lit-synapse.embedding.apiBase");
  bindSave(apiKeyInput, "extensions.zotero.zotero-lit-synapse.embedding.apiKey");
  bindSave(
    dimensionsInput,
    "extensions.zotero.zotero-lit-synapse.embedding.dimensions",
    true,
    true,
  );
  bindSave(timeoutInput, "extensions.zotero.zotero-lit-synapse.embedding.timeoutSeconds", true);
  ...
  modelInput?.addEventListener("change", async () => {
    if ((await getDataCompatibilityState()).embeddingIdentityLocked) {
      modelInput.value = String(
        Zotero.Prefs.get(
          "extensions.zotero.zotero-lit-synapse.embedding.model",
          true,
        ) ?? "",
      );
      await refreshDataCompatibilityLockUI(doc);
    ...
    const model = modelInput.value;
    Zotero.Prefs.set("extensions.zotero.zotero-lit-synapse.embedding.model", model, true);
      ...
      if (data && data.data && data.data.length > 0) {
        ...
        if (hasStoredVectors && storedDims && storedDims !== dims) {
          ...
          if (!embeddingLocks.detectedDimensions) {
            Zotero.Prefs.set("extensions.zotero.zotero-lit-synapse.embedding.detectedDimensions", dims, true);
          }
        } else {
          ...
          if (dims > 0) {
            ...
            if (
              ...
              Zotero.Prefs.set("extensions.zotero.zotero-lit-synapse.embedding.dimensions", dims, true);
            }
...
function getEmbeddingTimeoutSeconds(): number {
  try {
    const raw = Zotero.Prefs.get("extensions.zotero.zotero-lit-synapse.embedding.timeoutSeconds", true);
...
function updateEmbeddingServiceConfig() {
  try {
    ...
    const apiBase = Zotero.Prefs.get("extensions.zotero.zotero-lit-synapse.embedding.apiBase", true) || "";
    const apiKey = Zotero.Prefs.get("extensions.zotero.zotero-lit-synapse.embedding.apiKey", true) || "";
    const model = Zotero.Prefs.get("extensions.zotero.zotero-lit-synapse.embedding.model", true) || "";
    const dimensions = Zotero.Prefs.get("extensions.zotero.zotero-lit-synapse.embedding.dimensions", true);
...
function bindApiUsageStats(doc: Document) {
  ...
  initRateLimitValue(rpmInput, "extensions.zotero.zotero-lit-synapse.embedding.rpm", "60");
  initRateLimitValue(tpmInput, "extensions.zotero.zotero-lit-synapse.embedding.tpm", "150000");
  initRateLimitValue(costInput, "extensions.zotero.zotero-lit-synapse.embedding.costPer1M", "0.02");
  ...
  bindRateLimitSave(rpmInput, "extensions.zotero.zotero-lit-synapse.embedding.rpm");
  bindRateLimitSave(tpmInput, "extensions.zotero.zotero-lit-synapse.embedding.tpm");
  bindRateLimitSave(costInput, "extensions.zotero.zotero-lit-synapse.embedding.costPer1M", true);
...
function updateEmbeddingServiceRateLimits() {
  try {
    ...
    const rpm = Zotero.Prefs.get("extensions.zotero.zotero-lit-synapse.embedding.rpm", true);
    const tpm = Zotero.Prefs.get("extensions.zotero.zotero-lit-synapse.embedding.tpm", true);
    const costPer1M = Zotero.Prefs.get("extensions.zotero.zotero-lit-synapse.embedding.costPer1M", true);
    ...
    try {
      ...
      if (dimensionsEl) {
        if (stats.indexStats.storedDimensions) {
          ...
          const configuredDims = Zotero.Prefs.get("extensions.zotero.zotero-lit-synapse.embedding.dimensions", true);
