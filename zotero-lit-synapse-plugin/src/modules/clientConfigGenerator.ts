  ...
  private static readonly CLIENT_CONFIGS: ClientConfig[] = [
    {
      ...
      configTemplate: (port: number, serverName = "zotero-lit-synapse") => ({
        mcp_servers: {
          [serverName]: {
            type: "http",
      ...
      renderConfig: (port: number, serverName = "zotero-lit-synapse") => {
...
"Content-Type" = "application/json"`;
    ...
    },
    {
      ...
      renderConfig: (port: number) => {
        return `claude mcp add --transport http zotero-lit-synapse http://127.0.0.1:${port}/mcp`;
      },
      configLanguage: "bash",
      getInstructions: (port: number = 23120) => [
        ...
        "▶ 添加服务器（复制上方命令执行即可）",
        "──────────────────────────────────────────────────────────",
        `   claude mcp add --transport http zotero-lit-synapse http://127.0.0.1:${port}/mcp`,
        "",
        "   如需全局可用（所有项目）:",
        `   claude mcp add --transport http --scope user zotero-lit-synapse http://127.0.0.1:${port}/mcp`,
        "",
        "▶ 管理命令",
        "──────────────────────────────────────────────────────────",
        "   查看已添加:   claude mcp list",
        "   查看详情:     claude mcp get zotero-lit-synapse",
        "   移除服务器:   claude mcp remove zotero-lit-synapse",
    ...
    },
    {
      ...
      getInstructions: (port: number = 23120) => [
        ...
        `   qwen mcp add zotero-lit-synapse http://127.0.0.1:${port}/mcp -t http`,
        "",
        "2. Alternatively, add with custom headers and options:",
        `   qwen mcp add zotero-lit-synapse http://127.0.0.1:${port}/mcp \\`,
        "     -t http \\",
    ...
    },
    {
      ...
      configTemplate: (port: number, serverName = "zotero-lit-synapse") => ({
        name: serverName,
        description: "Zotero LitSynapse MCP Server - Research management and citation tools",
        transport: {
          type: "http",
  ...
  static generateConfig(clientName: string, port: number, serverName?: string): string {
    ...
    if (client.renderConfig) {
      return client.renderConfig(port, serverName || "zotero-lit-synapse");
    }

    const config = client.configTemplate(port, serverName || "zotero-lit-synapse");
  ...
  static generateFullGuide(clientName: string, port: number, serverName?: string): string {
    ...
    const actualServerName = serverName || "zotero-lit-synapse";
