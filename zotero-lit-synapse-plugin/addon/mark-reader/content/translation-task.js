...
var ZMRTranslationTaskWindow = {
  ...
  renderDocuments() {
    ...
    for (const item of this.snapshot?.documents || []) {
      ...
      option.value = item.attachmentIdentity;
  ...
  selectedDocument() {
    ...
    return this.snapshot?.documents.find((item) => item.attachmentIdentity === key);
  },
  ...
  renderGlossary(force = false) {
    ...
    if (
      !force &&
      this.glossaryDirty &&
      this.renderedGlossaryKey === documentItem?.attachmentIdentity
    ) {
      return;
    }
  ...
  saveGlossary() {
    ...
    const manual = this.renderedGlossaryQuery
      ...
      : visible;
    this.glossaryDirty = false;
    this.task.setDocumentGlossary(item.attachmentIdentity, manual, { replaceAI: true });
  },
