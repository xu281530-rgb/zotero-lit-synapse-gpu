...
for (const id of [
  "zotero-lit-synapse-wiki-button",
  "zotero-lit-synapse-wiki-panel",
  "zotero-lit-synapse-wiki-pages",
  "zotero-lit-synapse-wiki-claims",
  "zotero-lit-synapse-wiki-evidence",
  "zotero-lit-synapse-wiki-graph",
]) {
  assert.ok(panel.includes(id), `Wiki UI must define ${id}`);
}

for (const id of [
  "zotero-lit-synapse-wiki-terms-list",
  "zotero-lit-synapse-wiki-terms-detail",
]) {
...
assert.match(css, /#zotero-lit-synapse-wiki-panel/u);
...
assert.match(
  css,
  /\.zotero-lit-synapse-wiki-toolbarbutton\s+\.toolbarbutton-icon\s*\{[^}]*width:\s*30px;[^}]*height:\s*30px;/su,
  "Wiki toolbar icon must render at 30 by 30 pixels",
);
assert.match(
  css,
  /\.zotero-lit-synapse-wiki-toolbarbutton\s*\{[^}]*width:\s*36px;[^}]*height:\s*36px;/su,
  "Wiki toolbar button must contain the enlarged icon without clipping",
);
assert.match(panel, /zotero-tb-button zotero-lit-synapse-wiki-toolbarbutton/u);
assert.match(
  panel,
...
const firstRender = openWikiTab(fakeWindow, {
  type: "zotero-lit-synapse-wiki",
  title: "LLM 知识库",
  icon: "zotero-lit-synapse-wiki",
});
assert.equal(tabCalls.added.length, 1);
assert.equal(tabCalls.added[0].select, true);
assert.deepEqual(tabCalls.added[0].data, { icon: "zotero-lit-synapse-wiki" });
