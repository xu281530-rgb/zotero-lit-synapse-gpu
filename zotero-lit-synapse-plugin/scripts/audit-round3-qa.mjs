...
vectorStore.getChunksForItem = async () => chunks;
...
// This audit records present behavior, rather than asserting the desired fix.
const coverage = await read("MISSCOVR", [100, 101, 102]);
const missingNote = await note("MISSCOVR");
assert.equal(coverage.reading.deliveredChunks, 3);
assert.equal(coverage.reading.coverageComplete, true);
assert.equal(missingNote.records.length, 1);
assert.ok(!JSON.stringify(missingNote).includes("competing surface instability"));
assert.ok(!JSON.stringify(missingNote).includes("rapid cooling"));
results.push({ finding: "qa_record_omits_declared_chunks", declared: [100, 101, 102], recorded: "chunk 0 only", deliveredChunks: coverage.reading.deliveredChunks, coverageComplete: coverage.reading.coverageComplete, wikiDebt: coverage.wikiDebt });

const empty = await service.updateReadingNote({ libraryID: 1, itemKey: "EMPTYQA1", readChunkIds: [100, 101, 102], unchanged: true, unchangedReason: "already known" });
assert.equal(empty.reading.deliveredChunks, 3);
assert.equal(empty.reading.coverageComplete, true);
const emptyNote = await note("EMPTYQA1");
assert.equal(emptyNote.records.length, 1);
results.push({ finding: "first_qa_read_can_be_entirely_unchanged", deliveredChunks: empty.reading.deliveredChunks, coverageComplete: empty.reading.coverageComplete, records: emptyNote.records });
...
assert.equal(secondRead.wikiDebt.count, 0);
assert.match(secondRead.nextStep, /Carry on/);
...
assert.match(questionError, /not delivered/);
