...
await audit(
  ...
  async () => {
    ...
    assert.equal(after.body, original);
    assert.equal(controlledEmbeddingCalls, 0, "exact duplicate must not request an embedding");
    return {
      itemKey: item.key,
...
await audit(
  ...
  async () => {
    ...
    assert.equal(afterLatest, originalLatest);
    assert.ok(afterOlder.includes(record1));
    ...
    return {
      ...
      latestWasReplacedWithOlderEpisode: afterLatest === afterOlder,
...
await audit(
  ...
  async () => {
    ...
    let retryError;
    const reopenedDB = new DatabaseSync(path.join(rootDir, "notes.sqlite"));
    const reopenedStore = new WikiStore(adapt(reopenedDB));
    await reopenedStore.initialize();
    try {
      await new WikiService(reopenedStore).updateReadingNote(options);
    } catch (error) {
    ...
    assert.ok((await sessions.get(session.sessionId)).finalSynthesisAt);
    return {
      firstError,
      retryError,
      summaryPersisted: true,
      ledgerFinalSynthesisAt: (await sessions.get(session.sessionId)).finalSynthesisAt,
    };
  },
);
...
await audit(
  ...
  async () => {
    ...
    // A bounded pause overlaps callers without requiring both to enter a
    // critical section that the fixed implementation serializes.
...
await audit(
  ...
  async () => {
    ...
    await assert.rejects(target.setReadingExpert({
      libraryID: 1,
      itemKey: item.key,
      persona:
...
await audit(
  ...
  async () => {
    ...
    assert.equal(file.metadata.status, "skipped");
    return {
...
await audit("semantic duplicates retain the entire concluded file", async () => {
  const item = paper("NOTESEMA", 1);
  const original = appendMacroSummary(body(), summary);
  const attachment = await notes.ensureAttachment(item, original);
  const before = controlledEmbeddingCalls;
  const result = await service().updateReadingNote({ libraryID: 1, itemKey: item.key,
    readChunkIds: [0], readingRecord: "The imposed thermal gradient guides the orientation of the solidified grain array (chunk 0)." });
  assert.ok(result.readingNote.discarded);
  assert.ok(controlledEmbeddingCalls > before, "nonidentical text retains semantic comparison");
  assert.equal(await notes.read(attachment), original);
  return { fileUnchanged: true, semanticComparisonUsed: true };
});

await audit("header failure resumes without appending the record twice", async () => {
  const item = paper("NOTEHEAD", 2);
  const target = service();
  const write = target.notes.write.bind(target.notes);
  let writes = 0;
  target.notes.write = async (...args) => {
    if (++writes === 2) throw new Error("Injected header failure");
    return write(...args);
  };
  const options = { libraryID: 1, itemKey: item.key, readChunkIds: [0], readingRecord: record };
  await assert.rejects(target.updateReadingNote(options), /header failure/);
  const pending = await store.getNoteOperation(1, item.key);
  assert.equal(pending.ledgerDone, true);
  const retried = await service().updateReadingNote({ readingRecord: record, readChunkIds: [0], itemKey: item.key, libraryID: 1 });
  assert.equal(retried.recovered, true, "argument key order does not change operation identity");
  const attachment = await notes.getByKey(1, retried.readingNote.attachmentKey);
  assert.equal(parseAppendOnlyReadingNote(parseReadingNote(await notes.read(attachment)).body).records.length, 1);
  assert.equal(await store.getNoteOperation(1, item.key), null);
  return { recordCount: 1, recovered: true };
});

await audit("recovery refuses changed source and edited note content", async () => {
  const item = paper("NOTEVER", 1);
  const target = service();
  const integrate = sessions.recordIntegration.bind(sessions);
  sessions.recordIntegration = async () => { throw new Error("Injected ledger failure"); };
  const options = { libraryID: 1, itemKey: item.key, readChunkIds: [0], readingRecord: record };
  try { await assert.rejects(target.updateReadingNote(options), /ledger failure/); }
  finally { sessions.recordIntegration = integrate; }
  const pending = await store.getNoteOperation(1, item.key);
  const attachment = await notes.getByKey(1, pending.session.noteKey);
  const original = await notes.read(attachment);
  vectors.getDocumentRevision = async () => "changed-source";
  await assert.rejects(service().updateReadingNote(options), /revision changed/);
  assert.equal(await notes.read(attachment), original);
  vectors.getDocumentRevision = async () => "stable-round3";
  await notes.write(attachment, original + "\nExternal edit to retain.");
  await assert.rejects(service().updateReadingNote(options), /note changed/);
  assert.ok((await notes.read(attachment)).includes("External edit to retain."));
  return { sourceConflictRejected: true, editedContentRetained: true };
});

await audit("terminal recovery never stamps a newer note episode", async () => {
  const item = paper("NOTENEWR", 1);
  const old = await sessions.startOrContinue({ libraryID: 1, itemKey: item.key, title: "old", totalChunks: 1, mode: "qa", sourceVersion: "stable-round3" });
  const target = service();
  const original = await target.writeNote(item, old, body(), "reading");
  target.notes.write = async () => { throw new Error("Injected terminal failure"); };
  const options = { libraryID: 1, itemKey: item.key, outcome: "skipped" };
  await target.finishReading(options);
  const newer = await sessions.startOrContinue({ libraryID: 1, itemKey: item.key, title: "new", totalChunks: 1, mode: "qa", sourceVersion: "stable-round3" });
  const next = await notes.createNextAttachment(item, "New episode body.");
  await sessions.setNoteKey(newer.sessionId, next.key);
  const recovered = await service().finishReading(options);
  assert.equal(recovered.recovered, true);
  assert.equal(recovered.noteStatusWrite.attachmentKey, original.attachmentKey);
  assert.equal(await notes.read(next), "New episode body.");
  assert.equal((await sessions.get(newer.sessionId)).state, "reading");
  return { oldStatusRecovered: true, newerNoteUnchanged: true };
});
...
const resultPath = new URL(
  "../.scaffold/test-round3-notes-results.json",
  import.meta.url,
);
