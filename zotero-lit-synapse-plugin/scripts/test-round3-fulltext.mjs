...
await observe("fulltext partial readChunkIds silently integrates omitted passages", async () => {
  ...
  await assert.rejects(service.buildFromPaper({ libraryID: 31, userRequested: true, cursor: page.pagination.nextCursor }), /integrat|阅读记录/i);
  assert.equal(session.integratedChunks, 0);
  assert.deepEqual(pending, [0, 1, 2]);
  return { passed: "F01 partial QA record leaves fulltext batch pending", pending };
});
...
await observe("unsupported review leaves supported Claim retrievable after completed reading", async () => {
  ...
  assert.equal(Number(await db.valueQueryAsync("SELECT COUNT(*) FROM wiki_review_retractions WHERE session_id = ?", [sessionId])), 1);
...
await observe("unintegrated final page may be skipped by final synthesis", async () => {
  ...
  await assert.rejects(finalPass(34, "LASTPAGE"), /missing.*reading records|Reading records are missing/);
  ...
  assert.equal(after.deliveredBatches - after.integratedBatches, 1);
  assert.equal(after.finalSynthesisAt, null);
  assert.deepEqual(pending, [1]);
  // Independently guard closeout even for a legacy, inflated synthesis marker.
  await db.queryAsync("UPDATE wiki_reading_sessions SET final_synthesis_at = 1, concepts_recorded_at = 1, wiki_review_at = 1 WHERE session_id = ?", [sessionId]);
  const committed = await service.commit({ libraryID: 34, userInitiated: true, operationId: "last-page-omission", readingSessionId: sessionId, actions: [{ action: "SKIP", itemKey: "LASTPAGE", chunkIds: [1000, 1001], reason: "The supplied station-processing descriptions repeat the same method without another independent result for the thematic Wiki." }] });
  assert.notEqual(committed.readingSession.state, "committed");
  return { passed: "F02 synthesis and closeout reject missing records", pending };
});

await observe("unsupported review preserves support from another paper", async () => {
  const { sessionId } = await open(35, "MULTISRC", 1);
  const evidence = (itemKey) => ({ libraryID: 35, itemKey, chunkIdSnapshot: 1000,
    chunkTextHash: "hash", sourceContentHash: "source", sourceChunkSignature: "chunks",
    sourceResetGeneration: "reset", excerpt: "Directional solidification is discussed for the stations represented in these passages.",
    evidenceRole: "SUPPORTS", readDepth: "chunk_local" });
  const seeded = await store.commit({ libraryID: 35, userInitiated: true, actions: [
    { action: "CREATE_PAGE", ref: "p", canonicalTitle: "Multiple source processing" },
    { action: "ADD_CLAIM", ref: "c", pageId: "p", claimText: "Processing stations show an ordered response.", claimType: "mechanism",
      epistemicStatus: "corroborated", coverageLevel: "chunk_local", confidence: 0.9,
      evidence: [evidence("MULTISRC"), evidence("OTHERPAP")] },
  ] });
  const claimId = seeded.refs.c;
  await db.queryAsync("UPDATE wiki_reading_sessions SET wiki_review_at = 1, wiki_review = ? WHERE session_id = ?",
    [JSON.stringify({ ...axes, claimVerdicts: [{ claimId, verdict: "unsupported", basis: "This source does not support the claim." }] }), sessionId]);
  await store.commit({ libraryID: 35, userInitiated: true, readingSessionId: sessionId,
    actions: [{ action: "SKIP", reason: "Apply the completed source review." }] });
  const claim = await store.getClaim(claimId);
  assert.equal(claim.epistemicStatus, "supported");
  assert.deepEqual(claim.evidence.map((entry) => entry.itemKey), ["OTHERPAP"]);
  return { remainingSource: "OTHERPAP", status: claim.epistemicStatus };
});
