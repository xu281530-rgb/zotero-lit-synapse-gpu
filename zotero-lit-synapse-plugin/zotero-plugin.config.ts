          ...
          try {
            ...
            await writeFile(
              ...
              contents
                ...
                // The XPI basename is derived from `addonName`, so match it
                // rather than hard-coding it: stamp the version into whatever
                // filename the build produced.
                .replace(
                  new RegExp(
                    `/v${pkg.config.addonVersion.replace(/[.\\-]/g, "\\$&")}/([^/"]+?)\\.xpi`,
                    "g",
                  ),
                  `/v${pkg.config.addonVersion}/$1-${pkg.config.addonVersion}.xpi`,
                ),
              "utf8",
            );
          } catch (error: any) {
