...
describe("Round 3 full-text Wiki diagnostic in real Zotero", function () {
  ...
  diagnostic("a partial fulltext record must not silently integrate every delivered passage", async () => {
    ...
    assert.isTrue(Boolean(next.error), "A QA record must not clear the fulltext pagination gate.");
  });
  ...
  diagnostic("an unsupported review must not leave its only Claim supported and normally retrievable", async () => {
    ...
    assert.isNull(rejected, "Source reconciliation must commit successfully, not merely fail closed.");
    assert.equal(claim?.epistemicStatus, "unsupported");
    assert.isFalse(result.claims.some((entry: any) => entry.claimId === claimId));
  });
});
