...
describe("Wiki toolbar runtime", function () {
  ...
  it("selects and displays the Wiki tab when clicked", async function () {
    ...
    assert.equal(win.Zotero_Tabs.selectedType, "zotero-lit-synapse-wiki");

    for (let attempt = 0; attempt < 50; attempt += 1) {
      if (win.document.getElementById("zotero-lit-synapse-wiki-panel")) break;
      await Zotero.Promise.delay(100);
    }
    assert.equal(
      win.document.getElementById("zotero-lit-synapse-wiki-panel")?.parentElement?.id,
      openedTabID,
    );
    assert.isFalse(
      (win.Zotero_Tabs as any)._tabs.some(
        (tab: any) => tab.type === "zotero-lit-synapse-wiki" && tab.data === undefined,
      ),
    );
  });
});
