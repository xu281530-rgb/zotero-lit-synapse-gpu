...
const { values: options } = require('node:util').parseArgs({
  options: { source: { type: 'string' }, fixed: { type: 'boolean', default: false }, output: { type: 'string' } }
});
const sourceRoot = path.resolve(options.source || path.join(__dirname, '../source'));
const source = fs.readFileSync(path.join(sourceRoot, 'content/scripts/zotero-lit-synapse.js'), 'utf8');
const checks = [];
async function check(name, fn) {
  try { const details = await fn(); checks.push({ name, status:name.startsWith('CONFIRMED BUG:')?'BUG_REPRODUCED':'PASS', details: details ?? null }); }
...
(async()=>{
 ...
 await check(options.fixed ? 'HTTP rebind survives the old asynchronous stop callback' : 'CONFIRMED BUG: old stop callback marks a newly rebound server stopped',()=>{
  ...
  if(options.fixed){
    ...
    return {oldCallbackIgnored:true,reboundSocketClosed:true};
  }
  assert.equal(s.isServerRunning(),false);assert.equal(live.closed,false);
  s.stop();assert.equal(live.closed,false);
  return {reportedRunning:s.isServerRunning(),newSocketStillOpen:!live.closed,boundPortReported:s.getBoundPort(),stopFailedToCloseLiveSocket:true};
 });
 ...
 await check(options.fixed ? 'Reader translation caches are isolated by library and attachment key' : 'CONFIRMED BUG: translation caches alias across libraries with same attachment key',async()=>{
   const a={key:'ABCD2345',libraryID:1},b={key:'ABCD2345',libraryID:7};
   const da=await ri.getAttachmentDataDir(a), db=await ri.getAttachmentDataDir(b);
   if(options.fixed)assert.notEqual(da,db);else assert.equal(da,db);
   const ca=await ri.loadTranslationCache(a,{sourceHash:'paper-a',blocks:[]});ca.glossary.manual.push({source:'alpha',target:'A'});
   const cb=await ri.loadTranslationCache(b,{sourceHash:'paper-b',blocks:[]});
   if(options.fixed){
     assert.notStrictEqual(ca,cb);assert.equal(cb.sourceHash,'paper-b');assert.equal(cb.glossary.manual.length,0);
     assert.equal(ca.libraryID,1);assert.equal(cb.libraryID,7);
     return {separateDiskDirectories:true,separateMemoryObjects:true,glossaryIsolated:true};
   }
   assert.strictEqual(ca,cb);assert.equal(cb.sourceHash,'paper-a');
   return {sameDiskDirectory:da,sharedInMemoryObject:ca===cb,secondPaperSourceHash:cb.sourceHash,secondPaperInheritedGlossary:cb.glossary.manual};
 });
 ...
 const hc=context({
  setTimeout:()=>++timerID,setInterval:()=>++timerID,clearTimeout(){},clearInterval(){},SERVER_LISTENER_PREFS:{},refreshSemanticColumn(){},
 ...
 await check(options.fixed ? 'Index notifier retains group additions during an active build' : 'CONFIRMED BUG: ordinary group add during auto indexing is dropped instead of deferred',async()=>{
   hc.isAutoIndexing=true;await observer.notify('add','item',[99],{});
   const queuedDuringBuild=hc.pendingAutoUpdateKeys.size;
   assert.equal(queuedDuringBuild,options.fixed?1:0);
   hc.isAutoIndexing=false;await hc.triggerAutoIndexBuild();await new Promise(resolve=>setImmediate(resolve));
   assert.equal(hc.pendingAutoUpdateKeys.size,0);assert.deepEqual(periodicBuilds,options.fixed?[7,1]:[1]);
   return {groupLibraryID:7,queuedDuringBuild,periodicBuildLibraryIDs:periodicBuilds,groupChangeNotRecoveredByPeriodicScan:!options.fixed};
 });

 fs.writeFileSync(options.output ? path.resolve(options.output) : path.resolve(__dirname,options.fixed?'../test-results-fixed.json':'../test-results.json'),JSON.stringify(checks,null,2));
